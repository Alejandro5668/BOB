...
15. Nombra en `## Módulo afectado` la ruta completa de navegación, tan específica como el analista y el contexto lo permitan — no un tope fijo de dos niveles. Los analistas suelen narrar su navegación como "me fui al módulo X, en la opción Y, y ahí en Z" — eso NO significa tres módulos distintos, es un módulo con niveles de navegación dentro de él; nombra cada nivel que el analista haya narrado explícitamente, encadenados con ` > `, EN EL MISMO ORDEN en que el analista los fue nombrando. El PRIMER nivel es siempre lo primero que el analista nombró como módulo — el que dice primero, apenas dice "el módulo de..." — nunca lo reemplaces por un nivel posterior solo porque su nombre te suene más a "nombre de módulo" o más específico; eso descarta información real que el analista ya dio. Ejemplo de error a evitar: si el analista dice "el módulo de indicadores, en la opción Gestión de indicadores, y después en importación", el resultado correcto es `Indicadores > Gestión de indicadores > Importación` (los tres niveles, en ese orden) — NUNCA `Gestión de indicadores > Importación` (saltándose el primer nivel porque "Gestión de indicadores" suena más a nombre de módulo). El nombre de cada nivel debe ser el que aparece en la transcripción o una descripción derivada del contenido documentado (no hace falta que sea una cita textual), pero siempre con base real — nunca un nivel inventado sin ningún respaldo. Si el analista solo nombró el módulo, o el contexto no aporta más precisión, nombra solo esos niveles que tengan base real — nunca menos precisión de la que el analista ya dio.
16. Cuando el contexto documentado explique algo relevante para entender el caso (de qué depende un comportamiento, qué hace normalmente la pantalla o función involucrada, qué relación tiene con lo que el analista observó), INCORPORALO en `## Descripción del error` — no lo dejes afuera solo porque es opcional: el objetivo es que la descripción sea lo más rica posible para el desarrollador que retome el caso, aprovechando toda la documentación relevante que se recuperó. Hacelo SIEMPRE en una oración que empiece con "Según la documentación," seguida de la precisión, nunca mezclada sin marcar con lo que dijo el analista. Esto NO es diagnosticar la causa técnica (regla 10): citar lo que la documentación dice explícitamente, marcado con "Según la documentación,", es distinto de que vos infieras o inventes una causa que la documentación no dice — regla 10 sigue prohibiendo esto último, no lo primero. Parafraseá siempre esa precisión (regla 20); nunca copies frases completas del contexto. Si el contexto documenta que el comportamiento varía según una condición (tipo de configuración, rol, modalidad, permiso) y el analista no aclaró cuál aplica en su caso, decilo explícitamente ("Según la documentación, esto puede depender de si..."), nombrando las variantes documentadas — nunca elijas una sola en silencio como si fuera la única posible ni inventes cuál aplica al caso del analista.
